...
def _reduce_tag(tag, time_ago, minimum_timespan, simulate=False):
    ...
    values = tag.values.filter(timestamp_obtain__lte=latest_value).order_by('timestamp_obtain')
    if len(values) == 0:
        return 0
    ...
    for v in values[1:]:
        if v.timestamp_obtain < init_time + minimum_timespan:
            ...
            if simulate:
                print("SIM: leave alone {} ({} @ {})".format(v.tag.name, v.value, v.timestamp_obtain))
    cnt = len(to_delete)
    for v in to_delete:
        if not simulate:
            v.delete()
...
# It obtains schemes from base and convert it to optimized ones using following rules:
    ...
    for sch, settings in schemes.items():
        for t in settings["tags"]:
            ...
            for f in settings["filters"]:
                ...
                cnt += _reduce_tag(t, time_ago, minimum_timespan, simulate)
            reduct["count"] = cnt
